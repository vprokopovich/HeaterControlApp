...
IRequestPtrS RequestFactory::CreateRequest(const Json::Value& key, const Json::Value& value)
{
    ...
    do
    {
        ...
        else if ("RequestOnline" == value["action"].asString())
        {
            if (false == value["timestamp"].isDouble()) break;

            return std::make_shared<RequestOnline>(key.asString(), value["timestamp"].asDouble());
        }
    } while(false);
